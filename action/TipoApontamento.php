...
if($_POST)
{
	if ($_SESSION['validacaoform'] == base64_encode(date('d-m-Y')))
	{
		...
		switch ($acao)
		{
			case 'cadastrar/TipoApontamento' :
				...
				try
				{
					...
					$cadastro->cadastrarTipoApontamento();

					$cadastro->finalizarApp('cadastrar/TipoApontamento');

				}catch (Exception $e)
				{
					ClasseException::throwException($e,$_POST,'cadastrar/TipoApontamento');
				}
				break;

			case 'alterar/TipoApontamento' :
				...
				try
				{
					...
					$alteracao->alterarTipoApontamento();

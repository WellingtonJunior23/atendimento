...
class Cadastro extends Dados
{
	...
	public function cadastrarAtendimento()
	{
		try
		{
			...
			$this->dados['at_data_cadastro'] = ($this->dados['at_data_cadastro'] == '') ? date('Y-m-d') : ValidarDatas::dataBanco($this->dados['at_data_cadastro']);
				
				
			ValidarCampos::campoVazio($this->dados['at_paciente'],'Paciente');
				
			ValidarCampos::campoVazio($this->dados['at_descricao'],'Descrição');
			ValidarCampos::validaQtdCaracter($this->dados['at_descricao'], 20,'Descrição');
			...
			try
			{
				...
				$this->dados['at_codigo'] = $tbAtendimento->insert($this->dados);
	...
	public function cadastrarApontamento()
	{
		try
		{
			...
			ValidarCampos::campoVazio($this->dados['ap_descricao'],'Descrição');
			ValidarCampos::validaQtdCaracter($this->dados['ap_descricao'], 20,'Descrição');
				
			$this->dados['at_codigo'];
			$this->dados['sat_codigo'];
			$this->dados['ap_data_retorno'] =  ($this->dados['ap_data_retorno'] == '') ? date('Y-m-d') : ValidarDatas::dataBanco($this->dados['ap_data_retorno']);
				
			$this->dados['at_data_retorno'] = $this->dados['ap_data_retorno'];
			...
			try
			{
				...
				$tbAtendimento = new TbAtendimento();
				$tbAtendimento->updateAtendimento($this->dados);
	...
	public function cadastrarTipoDirecionamento()
	{

		try
		{
				
			ValidarCampos::campoVazio($this->dados['td_descricao'],'Descrição');
				
			try
			{
				$tbTipoDirecionamento = new TbTipoDirecionamento();

				$tbTipoDirecionamento->insert($this->dados);
	...
	public function cadastrarTipoAtendimento()
	{
		...
		try
		{
			...
			try
			{
				...
				$tbTipoAtendimento->insert($this->dados);
